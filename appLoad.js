      ...
      try {
        if (data.status === 200) {
          ...
          chrome.storage.sync.set({
            "userName_storeOwner": userName
          }, function() {
            console.log("Stored username");
          });
          chrome.storage.sync.set({
            "Authenticate": request.getResponseHeader('WWW-Authenticate')
          }, function() {
            console.log("Token Set");
          });
          chrome.storage.sync.set({
            "CLIENT_SECRET_KEY": request.getResponseHeader('CLIENT_SECRET_KEY')
          }, function() {
            console.log("Client Secret key Set");
          });
